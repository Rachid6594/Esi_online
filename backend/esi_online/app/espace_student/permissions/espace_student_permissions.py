...
# --- ReponseEtudiantQCM ---
class CanViewReponseEtudiantQCM(permissions.BasePermission):
    """Lecture (GET) : is_staff ou permission view_reponseetudiantqcm."""
    def has_permission(self, request, view):
        if request.method in ("GET", "HEAD", "OPTIONS"):
            return request.user.is_authenticated and (
                request.user.is_staff or request.user.has_perm("espace_student.view_reponseetudiantqcm")
            )
        return True


class CanCreateReponseEtudiantQCM(permissions.BasePermission):
    """Création (POST) : is_staff ou permission add_reponseetudiantqcm."""
    def has_permission(self, request, view):
        if request.method == "POST":
            return request.user.is_authenticated and (
                request.user.is_staff or request.user.has_perm("espace_student.add_reponseetudiantqcm")
            )
        return True


class CanUpdateReponseEtudiantQCM(permissions.BasePermission):
    """Modification (PUT, PATCH) : is_staff ou permission change_reponseetudiantqcm."""
    def has_permission(self, request, view):
        if request.method in ("PUT", "PATCH"):
            return request.user.is_authenticated and (
                request.user.is_staff or request.user.has_perm("espace_student.change_reponseetudiantqcm")
            )
        return True


class CanDeleteReponseEtudiantQCM(permissions.BasePermission):
    """Suppression (DELETE) : is_staff ou permission delete_reponseetudiantqcm."""
    def has_permission(self, request, view):
        if request.method == "DELETE":
            return request.user.is_authenticated and (
                request.user.is_staff or request.user.has_perm("espace_student.delete_reponseetudiantqcm")
            )
        return True
