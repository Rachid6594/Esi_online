...
from app.admin.services import AuditLogService
from app.admin.api.serializers import AuditLogSerializer
from app.admin.permissions.admin_permissions import (
    CanViewAuditLog, CanCreateAuditLog,
    CanUpdateAuditLog, CanDeleteAuditLog,
)
...
def auditlog_list(request):
    ...
    service = AuditLogService()
    obj = service.create(**serializer.validated_data)
    serializer = AuditLogSerializer(obj)
    return Response(serializer.data, status=status.HTTP_201_CREATED)
...
def auditlog_detail(request, pk: int):
    ...
    # PUT / PATCH
    partial = request.method == "PATCH"
    serializer = AuditLogSerializer(obj, data=request.data, partial=partial)
    serializer.is_valid(raise_exception=True)
    updated = service.update(pk, **serializer.validated_data)
    return Response(AuditLogSerializer(updated).data)
...
_decorate_crud_endpoints("user", UserSerializer)
_decorate_crud_endpoints("superadmin", SuperAdminSerializer)
_decorate_crud_endpoints("notification", NotificationSerializer)
_decorate_crud_endpoints("preferencenotification", PreferenceNotificationSerializer)
_decorate_crud_endpoints("auditlog", AuditLogSerializer)
